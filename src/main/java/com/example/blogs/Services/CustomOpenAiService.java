...
/**
 * AI service that uses OpenAI API to generate content for the social media platform.
 */
@Service
public class CustomOpenAiService {
    ...
    public CustomOpenAiService(PostService postService, CommentService commentService) {
        this.postService = postService;
        this.commentService = commentService;
    }
    
    private OpenAiService getOpenAiClient() {
        if (openAiClient == null) {
            openAiClient = new OpenAiService(apiKey, Duration.ofSeconds(60));
        }
        return openAiClient;
    }
    ...
    public String likeRandomPost() {
        ...
        // Create a prompt for the AI to choose a post to like
        StringBuilder postListBuilder = new StringBuilder();
        postListBuilder.append("Here are the posts available:\n\n");
        
        for (int i = 0; i < allPosts.size(); i++) {
            Post post = allPosts.get(i);
            postListBuilder.append("Post ID: ").append(post.getId()).append("\n");
            postListBuilder.append("Title: ").append(post.getTitle()).append("\n");
            postListBuilder.append("Content: ").append(post.getContent()).append("\n");
            postListBuilder.append("Author: ").append(post.getAuthor()).append("\n");
            postListBuilder.append("Current Likes: ").append(post.getLikesCount()).append("\n\n");
        }
        
        // Create a chat completion request
        List<ChatMessage> messages = new ArrayList<>();
        messages.add(new ChatMessage("system", 
            "You are an AI agent on a social media platform. Your task is to analyze the following posts " +
            "and choose ONE to like based on quality, relevance, and creativity."
        ));
        messages.add(new ChatMessage("user", 
            postListBuilder.toString() + "\n\n" +
        ...
        try {
            ...
            ChatCompletionResult result = getOpenAiClient().createChatCompletion(chatCompletionRequest);
    ...
    public String commentOnRandomPost() {
        ...
        // Create a chat completion request for comment generation
        List<ChatMessage> messages = new ArrayList<>();
        messages.add(new ChatMessage("system", 
        ...
        try {
            // Get AI's response
            ChatCompletionResult result = getOpenAiClient().createChatCompletion(chatCompletionRequest);
            String commentText = result.getChoices().get(0).getMessage().getContent().trim();
    ...
    public Post generatePost() {
        try {
            ...
            // Create a chat completion request for post generation
            List<ChatMessage> messages = new ArrayList<>();
            messages.add(new ChatMessage("system", 
            ...
            ChatCompletionResult result = getOpenAiClient().createChatCompletion(chatCompletionRequest);
            String generatedContent = result.getChoices().get(0).getMessage().getContent();
